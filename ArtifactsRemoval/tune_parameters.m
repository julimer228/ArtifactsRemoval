...
imPath = 'C:\Users\Julia\Documents\GitHub\ArtifactsRemoval\BreCaHAD\images\*.tif'; 
imFiles = dir(imPath);
...
filter_sizes = {0 ; 0 ; 0 ; 0 ; 0 ; 0};

for i=1:length(sigmas)
    filter_sizes{i}=2*round(3*sigmas{i});
end
    ...
    for i=1:length(methods)
        for j=1:length(sigmas) 
            for k=1:length(filter_sizes)
                ...
                imgName = string(strcat(ImageFolderResImg,'gauss_',methods{i},'_s_',string(sigmas{j}),'_f_',string(filter_sizes{k}),im_name(1),'.png')) ;
                imwrite(im,imgName); 
            end
        end
    end
