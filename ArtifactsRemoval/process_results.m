...
for qual_idx=1:length(quality)
    ...
    gauss_otsu = readtable(strcat("..\Results\Tables\Raw\", quality(qual_idx),"\otsu_gauss.csv"));
    gauss_multilevel=readtable(strcat("..\Results\Tables\Raw\", quality(qual_idx),"\multilevel_tresholding_gauss.csv"));
    gauss_fixed=readtable(strcat("..\Results\Tables\Raw\", quality(qual_idx),"\fixed_multilevel_tresholding_gauss.csv"));
    t_gauss={gauss_otsu, gauss_multilevel, gauss_fixed};
    
    % avg filter results
    avg_otsu = readtable(strcat("..\Results\Tables\Raw\", quality(qual_idx),"\otsu_avg.csv"));
    avg_multilevel=readtable(strcat("..\Results\Tables\Raw\", quality(qual_idx),"\multilevel_tresholding_avg.csv"));
    avg_fixed=readtable(strcat("..\Results\Tables\Raw\", quality(qual_idx),"\fixed_multilevel_tresholding_avg.csv"));
    ...
    folder_csv =strcat("..\Results\Tables\Mean\",quality(qual_idx),"\");
    ...
    for i=1:length(t_gauss_res)
        for j=1:length(columns_gauss)
             ...
             % save the heatmap
             exportgraphics(h, strcat(folder_heatmaps,"\Gauss\",methods{i},"_",column_name,".jpg"));
        end
    end
    ...
    for i=1:length(t_avg_res)
        for j=1:length(columns_avg)
             ...
             % save the heatmap
             exportgraphics(h, strcat(folder_heatmaps,"\Avg\",methods{i},"_",column_name,".jpg"))
        end
    end
end
%% Create boxplots
